  ...
  if (readingCnt >= maximumReadings) {
      ...
      if (WiFi.status() != WL_CONNECTED && millis() >= WIFI_TIMEOUT) {
        for (int i = 0; i < maximumReadings; i++) {
          Readings[i].temp1 = Readings[i+1].temp1;
          Readings[i].temp2 = Readings[i+1].temp2;
          Readings[i].time = Readings[i+1].time;
          Readings[i].volts = Readings[i+1].volts;
        }
          Readings[maximumReadings].temp1 = sensors.getTempCByIndex(0);       // Units °C
          Readings[maximumReadings].temp2 = sensors.getTempCByIndex(1);    // Units °C
